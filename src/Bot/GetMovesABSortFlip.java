...
public class GetMovesABSortFlip implements GetMoves{
	...
	public GetMovesABSortFlip(Evaluator evaluator, GetSearchNumber getSearchDepth){
	...
	private pathEvaluation opponentEvaluate2(Board board, Evaluator eval, int recursions, int sortThreshold, pathEvaluation alpha, pathEvaluation beta){
		if(recursions <= 0) return new pathEvaluation(eval.evaluate(board), 0);
		else{
			pathEvaluation minVal = new pathEvaluation(Integer.MAX_VALUE, recursions);
			Move[] legalMoves = board.getLegalMovesForOpponent();
	...
	public Stack<Move> getPlayerMoves(Board board, int time, int round, Move lastOpponentMove){
		...
		if(floodFillResult != -1){
			...
			for(Move move : legalMoves){
				if(!board.isLegalMoveForPlayer(move)) continue;
				board.makePlayerMove(move);
				pathEvaluation moveEvaluation = opponentEvaluate2(board, this.evaluator, recursions - 1, recursions - 3, alpha, beta);
				board.undoPlayerMove(move);
