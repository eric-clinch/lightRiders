...
public class GetMovesABCacheTreePrune implements GetMoves{
	
	private static int MIN_VAL = Integer.MIN_VALUE / 2;
	private static int MAX_VAL = Integer.MAX_VALUE / 2;
	...
	public GetMovesABCacheTreePrune(Evaluator evaluator, GetSearchNumber getSearchDepth, int pruneThreshold){
	...
	private PathEvaluation playerEvaluation(Board board, CacheTree thisTree, int depth, int maxDepth, PathEvaluation alpha, PathEvaluation beta){
		...
		PathEvaluation maxVal = new PathEvaluation(MIN_VAL, depth);
				...
				if(maxVal.compare(moveEvaluation) < 0){
					...
					if(alpha.compare(maxVal) < 0){
						...
						if(beta.compare(alpha) <= 0){
							...
							if(shouldSort) for(int j = i + 1; j < legalMovesLength; j++) edges[j] = new CacheTreeEdge(children[j], new PathEvaluation(MIN_VAL, depth));
							break; // prune
						}
					}
				}
			...
			for(int i = 0; i < numChildren; i++){
				...
				if(maxVal.compare(moveEvaluation) < 0){
					...
					if(alpha.compare(maxVal) < 0){
						alpha = maxVal;
						if(beta.compare(alpha) <= 0) {
							if(shouldSort) for(int j = i + 1; j < numChildren; j++) edges[j] = new CacheTreeEdge(children[j], new PathEvaluation(MIN_VAL, depth));
							break; // prune
						}
					}
				}
	...
	private PathEvaluation opponentEvaluation(Board board, CacheTree thisTree, int depth, int maxDepth, PathEvaluation alpha, PathEvaluation beta){
		...
		PathEvaluation minVal = new PathEvaluation(MAX_VAL, depth);
		CacheTreeEdge[] edges = null;
		boolean shouldSort = depth == sortThreshold || depth == sortThreshold - 1;
			...
			for(int i = 0; i < legalMovesLength; i++){
				...
				if(minVal.compare(moveEvaluation) > 0){
					...
					if(beta.compare(minVal) > 0){
						...
						if(beta.compare(alpha) <= 0){
							for(int j = i + 1; j < legalMovesLength; j++) children[j] = new CacheTreeChild(legalMoves[j], null);
							if(shouldSort) for(int j = i + 1; j < legalMovesLength; j++) edges[j] = new CacheTreeEdge(children[j], new PathEvaluation(MAX_VAL, depth));
							break; // prune
						}
					}
				}
			...
			for(int i = 0; i < numChildren; i++){
				...
				if(minVal.compare(moveEvaluation) > 0){
					...
					if(beta.compare(minVal) > 0){
						beta = minVal;
						if(beta.compare(alpha) <= 0) {
							if(shouldSort) for(int j = i + 1; j < numChildren; j++) edges[j] = new CacheTreeEdge(children[j], new PathEvaluation(MAX_VAL, depth));
							break; // prune
						}
					}
				}
	...
	public Stack<Move> getPlayerMoves(Board board, int time, int round, Move lastOpponentMove){
		...
		if(partitioned){
			...
			PathEvaluation alpha = new PathEvaluation(MIN_VAL, MIN_VAL);
			PathEvaluation beta = new PathEvaluation(MAX_VAL, MAX_VAL);
			
			if(currentTree == null){
				bestMove = Move.UP;
				PathEvaluation bestEval = new PathEvaluation(MIN_VAL, MIN_VAL);
